...
(function (ThisCouldBeBetter) {
    var Geometry;
    (function (Geometry) {
        class BoxAxisAligned extends Geometry.ShapeBase {
            constructor(center, size) {
                super();
                this.center = center || Geometry.Coords.create();
                this.size = size || Geometry.Coords.create();
                this._sizeHalf = Geometry.Coords.create();
                this._min = Geometry.Coords.create();
                this._max = Geometry.Coords.create();
                this._range = new Geometry.RangeExtent(0, 0);
            }
            static create() {
                return BoxAxisAligned.fromCenterAndSize(Geometry.Coords.create(), Geometry.Coords.create());
            }
            static default() {
                return BoxAxisAligned.fromCenterAndSize(Geometry.Coords.zeroes(), Geometry.Coords.ones());
            }
            ...
            static fromSize(size) {
                return new BoxAxisAligned(Geometry.Coords.zeroes(), size);
            }
            ...
            intersectWith(other) {
                ...
                var rangesForDimensions = [new Geometry.RangeExtent(0, 0), new Geometry.RangeExtent(0, 0), new Geometry.RangeExtent(0, 0)];
                var rangeOther = new Geometry.RangeExtent(0, 0);
                ...
                if (doAllDimensionsOverlapSoFar) {
                    var center = Geometry.Coords.create();
                    var size = Geometry.Coords.create();
                    for (var d = 0; d < rangesForDimensions.length; d++) {
            ...
            pointRandom(randomizer) {
                var min = this.min();
                var offset = Geometry.Coords.default().randomize(randomizer).multiply(this.size);
        ...
        Geometry.BoxAxisAligned = BoxAxisAligned;
