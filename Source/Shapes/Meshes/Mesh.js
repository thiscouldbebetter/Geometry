...
(function (ThisCouldBeBetter) {
    var Geometry;
    (function (Geometry) {
        class Mesh extends Geometry.ShapeBase {
            ...
            static boxOfSize(center, size) {
                var box = Geometry.BoxAxisAligned.fromCenterAndSize(center, size);
                var returnValue = Mesh.fromBox(box);
                return returnValue;
            }
            static cubeUnit(center) {
                if (center == null) {
                    center = Geometry.Coords.create();
                }
                var size = Geometry.Coords.fromXYZ(2, 2, 2);
                var returnValue = Mesh.boxOfSize(center, size);
                return returnValue;
            }
            static fromBox(box) {
                var sizeHalf = box.sizeHalf();
                var min = Geometry.Coords.fromXYZ(-sizeHalf.x, -sizeHalf.y, -sizeHalf.z);
                var max = Geometry.Coords.fromXYZ(sizeHalf.x, sizeHalf.y, sizeHalf.z);
                var vertexOffsets = [
                    // top
                    Geometry.Coords.fromXYZ(min.x, min.y, min.z),
                    Geometry.Coords.fromXYZ(max.x, min.y, min.z),
                    Geometry.Coords.fromXYZ(max.x, max.y, min.z),
                    Geometry.Coords.fromXYZ(min.x, max.y, min.z),
                    // bottom
                    Geometry.Coords.fromXYZ(min.x, min.y, max.z),
                    Geometry.Coords.fromXYZ(max.x, min.y, max.z),
                    Geometry.Coords.fromXYZ(max.x, max.y, max.z),
                    Geometry.Coords.fromXYZ(min.x, max.y, max.z),
                ];
                var faceBuilders = [
            ...
            static fromFace(face) {
                return Mesh.fromCenterFaceToExtrudeAndThickness(Geometry.Coords.create(), face, 1);
            }
            static fromCenterFaceToExtrudeAndThickness(center, faceToExtrude, thickness) {
                ...
                var faceBuilders = [];
                for (var z = 0; z < 2; z++) {
                    var offsetForExtrusion = Geometry.Coords.fromXYZ(0, 0, (z == 0 ? -1 : 1)).multiplyScalar(thicknessHalf);
                    var vertexIndicesTopOrBottom = [];
                    for (var v = 0; v < numberOfFaceVertices; v++) {
                        var vertexIndex = meshVertices.length;
                        if (z == 0) {
                            var vertexIndexNext = Geometry.NumberHelper.wrapToRangeMinMax(vertexIndex + 1, 0, numberOfFaceVertices);
            ...
            box() {
                if (this._boxAxisAligned == null) {
                    this._boxAxisAligned = Geometry.BoxAxisAligned.create();
                }
            ...
            vertices() {
                if (this._vertices == null) {
                    ...
                    for (var v = 0; v < this.vertexOffsets.length; v++) {
                        this._vertices.push(Geometry.Coords.create());
                    }
                }
            ...
            // clonable
            clone() {
                return new Mesh(this.center.clone(), Geometry.ArrayHelper.clone(this.vertexOffsets), Geometry.ArrayHelper.clone(this.faceBuilders));
            }
            overwriteWith(other) {
                this.center.overwriteWith(other.center);
                Geometry.ArrayHelper.overwriteWith(this.vertexOffsets, other.vertexOffsets);
                Geometry.ArrayHelper.overwriteWith(this.faceBuilders, other.faceBuilders);
                return this;
            }
            // Equatable
            ...
            static fromStringHumanReadable(meshAsString) {
                ...
                var center = Geometry.Coords.fromStringXxYxZ(centerAsString);
                lines = lines.slice(4);
                var textFaces = "Faces:";
                ...
                var vertexOffsets = vertexOffsetsAsStrings
                    .map(x => Geometry.Coords.fromStringXxYxZ(x.split(": ")[1]));
        ...
        class Mesh_FaceBuilder {
            ...
            toFace(meshVertices) {
                ...
                var returnValue = new Geometry.Face(faceVertices);
                return returnValue;
            }
            ...
            overwriteWith(other) {
                Geometry.ArrayHelper.overwriteWithNonClonables(this.vertexIndices, other.vertexIndices);
                return this;
            }
            // Serialization.
            toStringHumanReadable() {
        ...
        Geometry.Mesh_FaceBuilder = Mesh_FaceBuilder;
